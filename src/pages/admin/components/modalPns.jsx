...
import { addPNS, updatePNS } from "../../../services/database/pns";

const ModalPns = ({ isOpen, onClose, editData = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editData) {
        // Update existing PNS
        await updatePNS(editData.id, formData);
      } else {
        // Add new PNS
        await addPNS(formData);
      }
      onClose();
    } catch (error) {
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              {editData ? "Edit Data PNS" : "Tambah Karyawan Baru"}
            </h2>
            <p className="text-sm text-slate-500 mt-1">
              {editData
                ? "Perbarui informasi karyawan"
                : "Isi data karyawan PNS"}
            </p>
          </div>
          <button
            onClick={onClose}
